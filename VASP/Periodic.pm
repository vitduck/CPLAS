...
# check for a valid element form periodic table 
sub element_valid ( $self, $name ) { 
    return grep $name eq $_->[0],  $self->elements; 
} 

# atomic symbol => atomic number 
sub element_number ( $self, $symbol ) {  
    my ( $number ) =  grep $symbol eq $self->table->{$_}[0], $self->numbers; 

    return $number; 
}

# atomic name => atomic symbol 
sub element_symbol ( $self, $name ) { 
    my ( $symbol) = map $_->[0], grep $name eq $_->[1], $self->elements; 

    return $symbol; 
} 

# atomic symbol => atomic name 
sub element_name ( $self, $symbol ) {  
    my ( $element ) =  map $_->[1], grep $symbol eq $_->[0], $self->elements;  
